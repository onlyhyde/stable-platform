...
> **표준 기반**: ERC-4337 v0.7, ERC-7677 Paymaster Web Service
...
```
+------------------+     JSON-RPC      +-------------------+     JSON-RPC     +-----------+
|   Client/Wallet  | ----------------> |  Paymaster Proxy  | <--------------> |  Bundler  |
|  (SDK / dApp)    |  pm_getPaymaster* |   (이 서비스)      |                  |           |
+------------------+                   +-------------------+                  +-----------+
        |                                       |                                   |
        |                                       | Sign                              |
        v                                       v                                   v
+------------------+                   +-------------------+              +-------------------+
| Smart Account    |                   | Paymaster         |              | EntryPoint        |
| (ERC-4337)       |                   | Contracts (4종)   |              | (v0.7)            |
+------------------+                   +-------------------+              +-------------------+
```
...
### 4.1 UserOperation v0.7 (ERC-4337)

ERC-4337 v0.7은 두 가지 UserOperation 포맷을 지원한다: **Unpacked**(클라이언트 표준)와 **Packed**(온체인 효율).
...
#### 4.3.1 paymaster-proxy 해시 (mode byte 포함 버전)
...
#### 4.3.2 SDK 해시 (Packed UserOperation, mode byte 없음 버전)
...
#### 4.5.6 pm_supportedPaymasterTypes
...
#### 4.5.8 Context 파라미터를 통한 Paymaster 타입 라우팅
...
| 컨트랙트 | 주소 | 용도 |
|----------|------|------|
| **EntryPoint** | `0xEf6817fe73741A8F10088f9511c64b666a338A14` | ERC-4337 v0.7 엔트리포인트 |
