...
async function trySimulate(
  ...
  try {
    ...
    if (result.type === 'ValidationResult') {
      ...
      console.log(`  ${label}: ❌ FailedOp: "${result.reason}"`)
    } else if (result.type === 'FailedOpWithRevert') {
      console.log(`  ${label}: ❌ FailedOpWithRevert: "${result.reason}" inner=${result.inner}`)
    } else {
      console.log(`  ${label}: ❌ Unknown: ${result.reason}`)
    }
  }
}
...
async function main() {
  ...
  const accountGasLimits = concat([
    pad(toHex(500000n), { size: 16 }), pad(toHex(200000n), { size: 16 }),
  ]) as Hex
  const gasFees = concat([
    pad(toHex(1000000000n), { size: 16 }), pad(toHex(2000000000n), { size: 16 }),
  ]) as Hex
  const paymasterAndData = concat([
    CONFIG.paymaster, pad(toHex(200000n), { size: 16 }), pad(toHex(100000n), { size: 16 }),
  ]) as Hex
  ...
  const entryPointHash = (await publicClient.readContract({
    address: CONFIG.entryPoint, abi: EP_ABI, functionName: 'getUserOpHash', args: [packedOp],
  })) as Hex
  ...
  console.log('\nsigEip191:', sigEip191.slice(0, 20) + '...')
  console.log('EIP-191 wrapped hash:', hashMessage({ raw: entryPointHash }))

  // 2. Raw sign (direct ecSign without EIP-191 prefix)
  const sigRaw = await signer.sign({ hash: entryPointHash })
  console.log('sigRaw:', sigRaw.slice(0, 20) + '...')

  // Test all combinations
  console.log('\n--- Testing signature combinations ---')

  // Mode 0x02 + EIP-191 sign (SDK default)
  await trySimulate(publicClient, packedOp, concat(['0x02' as Hex, sigEip191]) as Hex, 'mode=0x02 + EIP-191')

  // Mode 0x02 + raw sign
  await trySimulate(publicClient, packedOp, concat(['0x02' as Hex, sigRaw]) as Hex, 'mode=0x02 + raw')

  // Mode 0x00 + EIP-191 sign
  await trySimulate(publicClient, packedOp, concat(['0x00' as Hex, sigEip191]) as Hex, 'mode=0x00 + EIP-191')

  // Mode 0x00 + raw sign
  await trySimulate(publicClient, packedOp, concat(['0x00' as Hex, sigRaw]) as Hex, 'mode=0x00 + raw')

  // Mode 0x01 + EIP-191 sign
  await trySimulate(publicClient, packedOp, concat(['0x01' as Hex, sigEip191]) as Hex, 'mode=0x01 + EIP-191')

  // Mode 0x01 + raw sign
  await trySimulate(publicClient, packedOp, concat(['0x01' as Hex, sigRaw]) as Hex, 'mode=0x01 + raw')
  ...
  await trySimulate(publicClient, packedOp, concat(['0x02' as Hex, dummySig]) as Hex, 'mode=0x02 + dummy')

  // Without paymaster, mode 0x02 + EIP-191
  console.log('\n--- Without paymaster ---')
  const packedOpNoPaymaster = { ...packedOp, paymasterAndData: '0x' as Hex }
  const hashNoPM = (await publicClient.readContract({
    address: CONFIG.entryPoint, abi: EP_ABI, functionName: 'getUserOpHash', args: [packedOpNoPaymaster],
  })) as Hex
  const sigNoPM_191 = await signer.signMessage({ message: { raw: hashNoPM } })
  const sigNoPM_raw = await signer.sign({ hash: hashNoPM })

  await trySimulate(publicClient, packedOpNoPaymaster, concat(['0x02' as Hex, sigNoPM_191]) as Hex, 'no-pm + mode=0x02 + EIP-191')
  await trySimulate(publicClient, packedOpNoPaymaster, concat(['0x02' as Hex, sigNoPM_raw]) as Hex, 'no-pm + mode=0x02 + raw')
  await trySimulate(publicClient, packedOpNoPaymaster, concat(['0x00' as Hex, sigNoPM_191]) as Hex, 'no-pm + mode=0x00 + EIP-191')
  await trySimulate(publicClient, packedOpNoPaymaster, concat(['0x00' as Hex, sigNoPM_raw]) as Hex, 'no-pm + mode=0x00 + raw')
}
