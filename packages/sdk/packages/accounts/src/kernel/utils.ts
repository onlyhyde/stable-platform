...
import type { Call, Validator } from '@stablenet/types'
import { CALL_TYPE, EXEC_MODE, MODULE_TYPE } from '@stablenet/types'
...
export function encodeExecutionMode(
  ...
  execMode: typeof EXEC_MODE.DEFAULT | typeof EXEC_MODE.TRY | typeof EXEC_MODE.DELEGATE = EXEC_MODE.DEFAULT
): Hex {
  // Construct mode bytes32
  const callTypeByte = callType === CALL_TYPE.SINGLE ? '00' : callType === CALL_TYPE.BATCH ? '01' : 'ff'
  const execModeByte = execMode === EXEC_MODE.DEFAULT ? '00' : execMode === EXEC_MODE.TRY ? '01' : 'ff'
...
export function encodeKernelExecuteCallData(calls: Call | Call[]): Hex {
  ...
  const mode = encodeExecutionMode(
    isSingle ? CALL_TYPE.SINGLE : CALL_TYPE.BATCH
  )

  // TypeScript needs help understanding the array is non-empty
  const firstCall = callArray[0]!

  const executionCalldata = isSingle
    ? encodeSingleCall(firstCall)
    : encodeBatchCalls(callArray)
 ...
 */
export function encodeKernelInitializeData(
  validator: Validator,
