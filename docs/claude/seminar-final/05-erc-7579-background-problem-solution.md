...
## 5.3 4337-7579 브리지 포인트

### 5.3.1 `validateUserOp`
...
#### 5.5.2.3 WeightedECDSAValidator
...
#### 5.5.3.3 RecurringPaymentExecutor
...
### 5.5.6 프로젝트 확장: Policy (Type 5) & Signer (Type 6)

#### 5.5.6.1 Policy 모듈 인터페이스
...
| Plugin          | 파일                                   | 용도            |
| --------------- | -------------------------------------- | --------------- |
| AutoSwapPlugin  | `erc7579-plugins/AutoSwapPlugin.sol`   | 자동 토큰 스왑  |
| MicroLoanPlugin | `erc7579-plugins/MicroLoanPlugin.sol`  | 소액 대출       |
| OnRampPlugin    | `erc7579-plugins/OnRampPlugin.sol`     | 법정화폐 온램프 |
...
## 5.6 EIP-7702 + ERC-7579 Kernel Delegate 조합
...
### 5.7.2 Fallback sender-context 이슈
...
### 5.8.2 비용 분석: 4337 오버헤드 vs 비즈니스 가치
...
- "7702+Kernel+7579 조합은 EOA 주소를 유지하면서 강력한 모듈형 확장을 제공하지만, delegate 변경/해제 리스크, 초기화 제어, 모듈 권한 관리에 대한 운영 정책이 반드시 수반되어야 한다."
- ERC-7579 표준 타입은 4가지(Validator/Executor/Fallback/Hook)이며, 본 코드베이스는 프로젝트 확장으로 Policy(5), Signer(6)을 추가해 사용한다. 각 모듈은 `onInstall()/onUninstall()`로 라이프사이클이 관리되며 필요에 따라 조합된다.
