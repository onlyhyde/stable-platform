...
- `modeSelector` (4 bytes): vendor/custom mode selector
- `modePayload` (22 bytes): 추가 데이터
...
- staticcall (`0xfe`): 스펙에 명시적 인코딩 정의 없음. callType만 정의되어 있으며, 구현 시 single과 동일 구조에서 value를 제외하는 방식이 합리적이나 스펙 외 해석임에 유의
...
- `installModule`:
  - 권한 통제 MUST
  - 필요 시 `module.onInstall(initData)` 호출 MUST
  - `ModuleInstalled` emit MUST
  - 이미 설치 or 초기화 실패 시 revert MUST
- `uninstallModule`:
  - 권한 통제 MUST
  - 필요 시 `module.onUninstall(deInitData)` 호출 MUST
...
Module type 구분의 보안적 근거(Rationale): 모듈 타입을 구분하지 않으면, validator로 설치된 모듈이 executor 권한으로 임의 트랜잭션을 실행할 수 있는 보안 문제가 발생한다. 따라서 계정은 모듈 타입별로 권한을 반드시 분리해야 한다.
...
- 다중 타입 모듈의 경우, `onInstall(data)`의 `data`에 moduleTypeId를 포함하여 타입별 초기화 로직을 분기할 수 있음
...
- 반환값 `uint256`은 ERC-4337 validation data 형식을 따름:
  - `validAfter` (6 bytes): 유효 시작 시간
  - `validUntil` (6 bytes): 유효 만료 시간
  - `authorizer` (20 bytes): 추가 인증자 주소 (0 = 유효, 1 = SIG_VALIDATION_FAILED)
