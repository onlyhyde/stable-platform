...
| 규칙 | 상태 | 요약 | 코드 위치 |
|------|------|------|-----------|
| R1 실행 엔트리 정합성 | **PASS** | STATIC 제외. BATCH, SINGLE, DELEGATECALL만 허용하여 ExecLib.execute()과 정합 | `Kernel.sol:649` |
| R2 `executeUserOp` 호환성 | **PASS** | `executeUserOp` onlyEntryPoint 강제 및 4337 경로 연결 확인 | `Kernel.sol:312-324, :361-365`, `EntryPoint.sol:249-251` |
...
| R5 Fallback sender 문맥 일치 | **FAIL** | **[구현 버그 2건]** (1) Kernel 20바이트 append vs Fallback 모듈 40바이트 기대 → §6.2, (2) TokenReceiverFallback 셀렉터가 Kernel 내장 함수와 충돌 → §6.3 | `ExecLib.sol:228-252`, `Kernel.sol:195-209` |
...
|-----------|------|-----------|------|-----------|
| C1 | 실행 모드 일관성 | R1 | **PASS** | `Kernel.sol:637-668` — STATIC 제외, ExecLib과 일치 |
| C2 | `executeUserOp` 브릿지 | R2 | **PASS** | `Kernel.sol:339-365` — 4337↔7579 브릿지 주석 기술 |
| C3 | validationData 형식 | R3 | **PASS** | `Kernel.sol:263-271` — `_intersectValidationData` 사용 |
| C4 | Hook 비결정성 | R4 | **PASS** | `Kernel.sol:353-360` — Hook 비결정론 경고 주석 기술 |
| C5 | Fallback sender 컨텍스트 | R5 | **FAIL** | **[구현 버그 2건]** (1) 20/40바이트 불일치 → §6.2, (2) Kernel 내장 함수 셀렉터 충돌 → §6.3 |
| C6 | nonce 계층 분리 | R6 | **PASS** | `Kernel.sol:273-279` — nonce 키 분리 주석 기술 |
...
| # | 요구사항 | 상태 | 근거 |
|---|----------|------|------|
| 1 | Account.validateUserOp은 EntryPoint만 호출 가능 | **PASS** | `Kernel.sol:284` onlyEntryPoint |
| 2 | validateUserOp은 서명 실패 시 SIG_VALIDATION_FAILED 반환 (revert 금지) | **PASS** | ValidationManager에서 처리 |
| 3 | missingAccountFunds를 EntryPoint에 전송 | **PASS** | `Kernel.sol:327-332` assembly call |
...
| 8 | paymasterAndData 파싱: [0:20] address, [20:36] valGas, [36:52] postOpGas | **PASS** | `BasePaymaster.sol:200` PAYMASTER_DATA_OFFSET |
...
| # | 요구사항 | 상태 | 근거 |
|---|----------|------|------|
| 1 | `execute(ExecMode, bytes)` 구현 | **PASS** | `Kernel.sol:406` |
| 2 | `executeFromExecutor(ExecMode, bytes)` 구현 | **PASS** | `Kernel.sol:383` |
| 3 | `installModule` / `uninstallModule` 구현 | **PASS** | `Kernel.sol:420-486, 525-583` |
| 4 | `isModuleInstalled` 구현 (타입 1-4) | **PASS** | `Kernel.sol:601-629` |
| 5 | `supportsModule` 구현 | **PASS** | `Kernel.sol:585` |
| 6 | `supportsExecutionMode` 구현 | **PASS** | `Kernel.sol:646-668` |
| 7 | `accountId` 구현 | **PASS** | `Kernel.sol:631-633` |
...
| 10 | installModule 시 이미 설치된 모듈 guard | **PASS** | `Kernel.sol:429-430, 451-452, 462-463` |
| 11 | uninstallModule 시 미설치 모듈 guard | **PASS** | `Kernel.sol:533-534, 538-539, 545-546` |
...
- **수정 내용**: `Kernel.sol:649`에서 CALLTYPE_BATCH, CALLTYPE_SINGLE, CALLTYPE_DELEGATECALL만 허용하도록 수정. CALLTYPE_STATIC 올바르게 제외.
...
- **수정 내용**: `Kernel.sol:614-628`에서 MODULE_TYPE_HOOK 분기 추가. rootValidator의 hook을 기본 확인하고, additionalContext(21 bytes)로 특정 validator의 hook도 조회 가능.
...
### 6.3 미해결 — IMPLEMENTATION BUG (2번째: 1건)
...
|------|------|
| **분류** | IMPLEMENTATION BUG |
| **위치** | `Kernel.sol:195-209` (내장 pure 함수) vs `TokenReceiverFallback.sol:129-214` (fallback 모듈 구현) |

**문제 설명**:

Kernel 컨트랙트에 다음 함수가 **explicit `pure` 함수**로 직접 정의되어 있다:
...
Solidity는 explicit 함수를 `fallback()` 보다 먼저 매칭하므로, 이 셀렉터들은 **절대 fallback 경로로 라우팅되지 않는다**.

**영향 범위**:

| TokenReceiverFallback 함수 | 셀렉터 | 도달 가능 여부 |
|---|---|---|
| `onERC721Received` | `0x150b7a02` | **불가** — Kernel 내장 함수 우선 |
| `onERC1155Received` | `0xf23a6e61` | **불가** — Kernel 내장 함수 우선 |
| `onERC1155BatchReceived` | `0xbc197c81` | **불가** — Kernel 내장 함수 우선 |
| `tokensReceived` (ERC-777) | `0x0023de29` | **가능** — Kernel에 미정의, fallback 경유 |

결과: TokenReceiverFallback의 화이트리스트/블랙리스트/로깅 기능이 **ERC-721/1155에 대해 사실상 dead code**이다. 이 모듈을 설치해도 ERC-721/1155 수신 시 Kernel의 내장 `pure` 함수가 항상 실행되어, 토큰 필터링이나 수신 로깅이 동작하지 않는다.

**수정 옵션** (택 1):

| 옵션 | 수정 대상 | 내용 |
|------|-----------|------|
| **A** | `Kernel.sol` | 내장 `onERC721Received/onERC1155Received/onERC1155BatchReceived` 제거. 기본 토큰 수신은 TokenReceiverFallback 설치로 대체 |
| **B** | `Kernel.sol` | 내장 함수에서 fallback 모듈 설치 여부를 확인하고, 설치된 경우 fallback으로 위임 |
| **C** | `TokenReceiverFallback.sol` | ERC-721/1155 지원을 제거하고 ERC-777 전용으로 축소. 문서에 Kernel 내장 함수 사용을 명시 |

**스펙 관점**: ERC-7579는 fallback 모듈 셀렉터가 계정의 기존 함수와 충돌할 경우의 동작을 명시하지 않으므로 SPEC VIOLATION은 아니다. 그러나 모듈 설치(`installModule`)가 성공함에도 해당 모듈이 실제로 호출되지 않는 것은 모듈 시스템의 기본 기대를 위반하는 구현 버그이다.

**테스트 케이스**:
- `test_TokenReceiverFallback_Installed_But_KernelBuiltinTakesPriority()`
- `test_TokenReceiverFallback_ERC777_tokensReceived_ReachesFallback()`
- `test_Kernel_OnERC721Received_IgnoresWhitelistBlacklist()`
...
|---|----------|-----------|---------------|------|
| 1 | 악의적 onUninstall revert로 모듈 제거 방해 | `7579스펙:491` | `Kernel.sol:580` — `ModuleLib.uninstallModule()`이 실패 시에도 revert하지 않고 이벤트로 추적 | 모듈이 자신의 제거를 차단할 수 없음 |
| 2 | 단일 타입 모듈 교체 시 잔여 상태 | `7579스펙:494` | fallback은 `target != address(0)` guard(`Kernel.sol:462`)로 직접 교체 불가. uninstall 선행 필요. 이전 모듈의 잔여 상태 자동 정리는 미구현 | 운영 시 수동 정리 필요 |
| 3 | self-call 배치를 통한 설정 함수 권한 우회 | `7579스펙:495` | `installModule`에 `onlyEntryPointOrSelfOrRoot` 적용(`Kernel.sol:424`). executor context에서의 self-call은 `ExecutorCannotCallSelf`(`Kernel.sol:112`)로 차단됨 | `initialize()` 등 정당한 self-call은 허용 |
...
| **P0-4** | **TokenReceiverFallback 셀렉터 충돌 (Kernel 내장 함수 우선)** | **IMPLEMENTATION BUG** | **즉시 수정 필요** |
| P1-1 | Paymaster validation 상태변경 정책 | BUNDLER COMPAT | Public mempool 대응 시만 |
| ~~P1-2~~ | ~~Envelope nonce 정책 통일~~ | — | **NOT AN ISSUE** |
| ~~P2-1~~ | ~~`MIN_DEPOSIT` 운영 정책 정리~~ | — | **DONE** |
...
**대상 파일** (택 1):
- **옵션 A**: `poc-contract/src/erc7579-smartaccount/Kernel.sol` — 내장 `onERC721Received/onERC1155Received/onERC1155BatchReceived` 제거
- **옵션 B**: `poc-contract/src/erc7579-smartaccount/Kernel.sol` — 내장 함수에서 fallback 모듈 설치 여부 확인 후 위임
- **옵션 C**: `poc-contract/src/erc7579-fallbacks/TokenReceiverFallback.sol` — ERC-721/1155 지원 제거, ERC-777 전용 축소
...
| 5 | `TokenReceiverFallback` 설치 후 ERC-721/1155 수신 시 fallback 모듈 실행 확인 | P0-4 |
...
| 환경 | 권장 구성 |
|------|-----------|
| **Trusted bundler (자체 인프라)** | P0-3 Fallback sender 수정 + P0-4 셀렉터 충돌 수정 후 사용 가능 |
| **Public mempool** | (1) Fallback sender 수정, (2) 셀렉터 충돌 수정, (3) Paymaster를 EntryPoint에 스테이킹, (4) Permit2Paymaster는 사전 approve 경로 사용, (5) senderNonce를 postOp으로 이동 검토 |
...
| 스펙 간 직접 충돌 | **0건** |
| 구현 버그 | **2건** (Fallback sender context 20/40 byte 불일치, TokenReceiverFallback 셀렉터 충돌) |
...
| **P0** | TokenReceiverFallback 셀렉터 충돌 (§6.3) | IMPLEMENTATION BUG — 즉시 수정 |
...
- [ ] P0-4 TokenReceiverFallback 셀렉터 충돌 수정 완료
...
#### 11.2.4 Bundler 검증 파이프라인 — PASS (주의사항 있음)
...
#### 11.2.5 SDK 모듈 레지스트리 — 주의사항

- SDK의 모듈 레지스트리에 `TokenReceiverFallback`이 등록되어 있음
- P0-4(§6.3)에 의해 ERC-721/1155 관련 기능이 사실상 동작하지 않으므로, P0-4 수정 전까지 SDK 레벨에서도 해당 모듈 설치 시 **경고 또는 제한** 추가 권장
...
| 해시 계산 (서명 검증) | **PASS** | domainSeparator, userOpCoreHash, envelope 해시 모두 일치 |
| 시간 단위 일관성 | **PASS** | 전체 Unix timestamp 사용 |
| Bundler 검증 규칙 | **PASS** (주의사항 2건) | Public mempool 운영 시 재시뮬레이션/모드 분리 보완 필요 |
| SDK 모듈 레지스트리 | **주의** | P0-4 수정 전 TokenReceiverFallback 설치 경고 필요 |
