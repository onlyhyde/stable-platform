...
4. Bundler가 `EntryPoint.handleOps()`(또는 `handleAggregatedOps`) 호출
5. EntryPoint가 검증 단계 수행
   - 계정 미배포 시 `initCode`로 배포 (이미 배포된 경우 initCode 무시, v0.9에서 `IgnoredInitCode` 이벤트 발행)
...
flowchart TB
    EP[EntryPoint]
    EP --> H["handleOps, handleAggregatedOps"]
    EP --> G[getUserOpHash]
    EP --> SA2[getSenderAddress]
    H --> V[Validation]
    H --> X[Execution]
    H --> ST[Gas Settlement]
```
...
- `getUserOpHash(PackedUserOperation calldata userOp) → bytes32`: EIP-712 기반 서명 대상 해시
- `getSenderAddress(bytes calldata initCode)`: counterfactual 주소 조회 (revert로 주소 반환)
...
시뮬레이션:
- v0.7 이후(v0.8, v0.9 포함) 별도 `simulateValidation` 함수가 없음
- Bundler는 `handleOps()`를 **view/trace call**로 호출하여 시뮬레이션 수행

검증 헬퍼:
- `delegateAndRevert(address target, bytes calldata data)`: state override 미지원 네트워크에서 검증용 delegatecall 수행 후 `DelegateAndRevert(bool, bytes)` 에러로 결과 반환
- `senderCreator() → ISenderCreator`: SenderCreator 인스턴스 조회 (v0.8부터 public)
...
| 항목 | `handleOps` | `handleAggregatedOps` |
|---|---|---|
...
**Block Number Mode**: `validUntil`과 `validAfter`의 highest bit를 1로 설정하면 timestamp 대신 **block number** 기준으로 동작한다. 동일 UserOperation 내에서 timestamp과 block number를 혼용할 수 없다 (둘 다 같은 모드여야 함).
...
1. 전체 `handleOps`에 대해 `debug_traceCall` 실행 SHOULD
2. 검증 규칙(opcode/storage) 적용
3. revert 발생 시 trace로 원인 entity 식별
4. 규칙 위반 시 revert로 처리
5. 문제 UserOp을 번들과 mempool에서 제거
6. factory/paymaster 원인 + sender unstaked → factory/paymaster 차단
7. factory/paymaster 원인 + sender staked → sender 차단
8. 성공하거나 유효 UserOp이 없을 때까지 반복
...
sequenceDiagram
    ...
    alt initCode > 20B (초기화 필요)
        ...
        SA-->>EP: emit EIP7702AccountInitialized
    end
...
flowchart TB
    ...
    subgraph Error Types
        ...
        E3["PostOpReverted(returnData)"]
        E4["SignatureValidationFailed()"]
    end
...
sequenceDiagram
    ...
    rect rgb(220, 240, 220)
        ...
        opt 이미 배포 + initCode 제공 (v0.9)
            EP->>EP: emit IgnoredInitCode
        end
        opt EIP-7702 초기화 (v0.9)
            EP->>EP: emit EIP7702AccountInitialized
        end
    end
...
EntryPoint가 발행하는 이벤트:

| 이벤트 | 설명 |
|--------|------|
| `UserOperationEvent` | UserOp 실행 결과 (성공/실패, 가스 사용량, 수수료 등) |
| `AccountDeployed` | initCode를 통한 신규 계정 배포 |
| `UserOperationRevertReason` | 실행 단계에서 revert 발생 시 사유 |
| `PostOpRevertReason` | `postOp` 호출에서 revert 발생 시 사유 |
| `UserOperationPrefundTooLow` | prefund가 실제 가스비보다 부족한 경우 |
| `BeforeExecution` | 검증 단계 완료 후 실행 단계 진입 시그널 |
| `IgnoredInitCode` | 이미 배포된 계정에 initCode 제공 시 무시됨 (v0.9) |
| `EIP7702AccountInitialized` | EIP-7702 delegate 초기화 완료 시 발행 (v0.9) |
...
- `validateSignatures`: 집계된 서명을 일괄 검증 (EntryPoint에서 호출)
- `validateUserOpSignature`: 개별 UserOp의 서명 유효성 검증
- `aggregateSignatures`: 여러 UserOp의 서명을 하나로 집계
- 모든 `validateSignatures` 호출이 EntryPoint에서 오는지 MUST 검증

> **참고**: ERC-4337 스펙은 Aggregator의 전체 설계를 "outside the scope of this proposal"로 명시. 위 인터페이스는 스펙이 참조하는 함수명과 참조 구현(eth-infinitism)을 기반으로 정리한 것이며, 정확한 파라미터는 구현체에 따라 다를 수 있다.
...
| `initCode` | 조건부 | 배포된 계정이면 `0x`, 미배포면 `factory(20B)+factoryCalldata`. v0.9: 이미 배포된 계정에 initCode 제공 시 무시 (`IgnoredInitCode` 이벤트 발행) |
...
5. on-chain 제출
   - `handleOps(ops, beneficiary)` 또는 `handleAggregatedOps`
