    ...
    <Card>
      <CardContent className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <p className="text-sm" style={{ color: 'rgb(var(--muted-foreground))' }}>{title}</p>
          <p className="text-2xl font-bold" style={{ color: 'rgb(var(--foreground))' }}>{value}</p>
          {change !== undefined && (
            <p
              className="text-sm"
              style={{ color: isPositive ? 'rgb(var(--success))' : 'rgb(var(--destructive))' }}
            >
              {isPositive ? '+' : ''}{change}% {changeLabel || 'from last month'}
            </p>
          )}
        </div>
      </CardContent>
