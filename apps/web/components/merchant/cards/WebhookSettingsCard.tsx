    ...
    <Card>
      ...
      <CardContent>
        {showAddForm && (
          ...
          >
            <h4 className="font-medium mb-4" style={{ color: 'rgb(var(--foreground))' }}>New Webhook Endpoint</h4>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {AVAILABLE_EVENTS.map((event) => (
                    ...
                    >
                      ...
                      />
                      <span className="text-sm" style={{ color: 'rgb(var(--foreground) / 0.8)' }}>{event.label}</span>
                    </label>
                  ))}
                </div>
              </div>
              <div className="flex justify-end">
                <Button onClick={handleAddEndpoint} disabled={isLoading || !newUrl || selectedEvents.length === 0}>
          ...
          <div className="space-y-4">
            {endpoints.map((endpoint) => (
              ...
              >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span
                        ...
                        style={{ backgroundColor: endpoint.active ? 'rgb(var(--success))' : 'rgb(var(--muted-foreground) / 0.5)' }}
                      />
                      <code className="text-sm font-mono break-all" style={{ color: 'rgb(var(--foreground) / 0.8)' }}>
                        {endpoint.url}
                      </code>
                    </div>
                    <p className="text-xs" style={{ color: 'rgb(var(--muted-foreground))' }}>
                      Created: {endpoint.createdAt.toLocaleDateString()}
                      {endpoint.lastTriggered && ` | Last triggered: ${endpoint.lastTriggered.toLocaleDateString()}`}
                    </p>
                  </div>
                  <Toggle
                ...
                <div className="mb-3">
                  <p className="text-xs font-medium mb-1" style={{ color: 'rgb(var(--muted-foreground))' }}>Events:</p>
                ...
                <div className="mb-3">
                  <p className="text-xs font-medium mb-1" style={{ color: 'rgb(var(--muted-foreground))' }}>Signing Secret:</p>
