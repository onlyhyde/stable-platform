...
import { getDocSection, getDocArticle, docSections } from '@/lib/docs'
...
export default function ArticlePage() {
  ...
  return (
    <div className="max-w-3xl">
      ...
      {/* Navigation */}
      <div className="flex items-center justify-between mt-12 pt-6 border-t" style={{ borderColor: 'rgb(var(--border))' }}>
        {prevArticle ? (
          <Link
...
function parseMarkdown(content: string): string {
  return content
    // Headers
    .replace(/^### (.*$)/gim, '<h3 class="text-lg font-semibold mt-6 mb-3" style="color: rgb(var(--foreground))">$1</h3>')
    .replace(/^## (.*$)/gim, '<h2 class="text-xl font-semibold mt-8 mb-4" style="color: rgb(var(--foreground))">$1</h2>')
    .replace(/^# (.*$)/gim, '<h1 class="text-2xl font-bold mb-6" style="color: rgb(var(--foreground))">$1</h1>')
    // Bold
    .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
    // Italic
    .replace(/\*(.*?)\*/g, '<em>$1</em>')
    // Code blocks
    .replace(/```(\w+)?\n([\s\S]*?)```/g, '<pre class="p-4 rounded-lg overflow-x-auto my-4" style="background-color: rgb(var(--secondary))"><code class="text-sm font-mono" style="color: rgb(var(--foreground))">$2</code></pre>')
    // Inline code
    .replace(/`([^`]+)`/g, '<code class="px-1.5 py-0.5 rounded text-sm font-mono" style="background-color: rgb(var(--secondary)); color: rgb(var(--primary))">$1</code>')
    // Links
    .replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" class="text-primary hover:underline">$1</a>')
    // Unordered lists
    .replace(/^\- (.*$)/gim, '<li class="ml-4" style="color: rgb(var(--muted-foreground))">$1</li>')
    // Ordered lists
    .replace(/^\d+\. (.*$)/gim, '<li class="ml-4" style="color: rgb(var(--muted-foreground))">$1</li>')
    // Tables
    .replace(/\|(.+)\|/g, (match) => {
